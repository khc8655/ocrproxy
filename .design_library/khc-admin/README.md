...
The following rules are derived from recent audits of OCRProxy admin interfaces. They are mandatory for any AI agent generating dashboard UI with this design system.

### Non-negotiable rules

1. **No emoji in UI text.** This includes buttons, labels, titles, toasts, badges, empty states, tables, and navigation items. Emoji are not part of KHC Admin's visual language.
2. **No exclamation marks, interjections, or marketing phrases.** Copy must read like technical documentation, not a landing page.
3. **No decorative gradients except the logo gradient.** The only allowed gradient is the navigation logo square: `linear-gradient(135deg, primary-500, primary-700)`.
4. **Do not invent token values.** Use only the tokens defined in `colors_and_type.css`. If a value is not in the token file, it is not part of this design system.

### Token enforcement

These tokens are frequently misused. Reference this table before assigning sizes or radii.

| Token / element | Value | Token name |
|---|---|---|
| Body text | 14px | `--font-size-body` |
| H1 | 24px | `--font-size-h1` |
| H2 | 20px | `--font-size-h2` |
| H3 | 16px | `--font-size-h3` |
| Small radius (button, input) | 2px | `--radius-sm` |
| Medium radius (card) | 4px | `--radius-md` |
| Large radius (panel) | 8px | `--radius-lg` |
| Extra large radius (top-level container) | 12px | `--radius-xl` |
| App bar height | 48px | `--size-button-lg` / navigation spec |
| Button height | 32px | `--size-button-sm` |
| Input height | 40px | `--size-input` |
| Card padding | 20px 24px | — |
| Table cell padding | 10px 12px | — |

**Important:** Badge is the **only** pill-shaped component (10px radius). Do not use the pill radius on buttons, tags, chips, or labels outside of Badge.

### Copy patterns

Use these patterns when generating labels, titles, and status text.

- **Navigation items:** noun phrases at a consistent level of abstraction.  
  Good: `"概览"`, `"配置"`, `"密钥"`, `"日志"`.
- **Action buttons:** verb-object structure, 2–6 Chinese characters.  
  Good: `"保存配置"`, `"探活"`, `"清空冷却"`, `"删除"`.
- **Status labels:** 2–4 characters. Progressive states end with `"中"` (`"运行中"`, `"排队中"`). Completed states do not (`"已完成"`, `"失败"`, `"已取消"`).
- **Section titles:** concise noun phrases. No decorative prefixes such as `"实时"`, `"智能"`, `"快捷"` unless they are part of the domain noun.

### Common agent mistakes

| Before (incorrect) | After (correct) | Problem fixed |
|---|---|---|
| `[emoji] OCRProxy` | `OCRProxy` | 移除 emoji |
| `[装饰] 边缘中转状态总览` | `状态总览` | 移除装饰前缀 |
| `实时探测当前处理请求的 EdgeOne 边缘 CDN 节点及其实际出网公网 IP（防止单 IP 429 限流）` | `出口 IP` | 精简冗长说明 |
| `[emoji] 重新探测节点 IP` | `探测 IP` | 移除 emoji 与副词 |
| `[emoji] 清空全部冷却状态` | `清空冷却` | 移除 emoji 与冗余宾语 |
| `配置已成功写入 EdgeOne KV 并向全网广播生效` | `配置已保存` | 精简成功叙述 |
| `[emoji] 连接成功` | `连接成功` | 移除 emoji |
| `暂无错误记录，运行状态优异 [emoji]` | `暂无错误记录` | 移除 emoji 与营销词 |
...
### Component usage reminders

- **Button:** 32px height, 2px radius, hover via `filter: brightness()`, no icons unless essential.
- **Card:** 4px radius, 1px border, whisper shadow, hover only shadow-2 (no border color change).
- **Table:** gray header from page surface color, no zebra stripes, rows separated by bottom border, padding 10px 12px.
- **Badge:** pill only, 50-level background + 500-level text, five semantic states.
- **Navigation:** 48px sticky bar, no emoji in nav labels.
