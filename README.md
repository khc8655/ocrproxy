...
| **后台界面自适应** | 隐藏 KB 候选挂载区与 KB 4项入库超时，只展示 Agent 模型管理、Agent 监控与接入指南 | 隐藏 Agent 模型区与 Agent 对话超时，只展示 4 大虚拟模型挂载、KB 入库超时与 Dify 接入指南 | **纯模型管理**（Agent 模型 + KB 虚拟模型，供应商与 Key 由 EdgeOne Vault 统一中枢按需点选下发） |
