...
本服务支持两种路由模式，共用同一组 `/v1/*` 接口，并在后台监控与统计中彻底解耦：
...
- **`/v1/models` 只返回 `agent_models` 的真实模型**，不包含 4 个虚拟别名
- **KB 入库模式**：chat 始终禁用思考且**始终快速**（强制非流式 + `chat_fast_timeout` 短超时）
...
- **请求级 400 精准短路**：当上游返回请求级 400（参数不受支持、上下文超长、内容审核等）时，直接原样返回响应并终止重试，彻底杜绝在同供应商内部来回切换浪费网络开销与延时
